/*
 * Generates the TypeScript ABIs from the monorepo's Foundry artifacts (../out).
 * Re-run after every `forge build` that changes the interfaces:
 *   npm run abis
 */
...
for (const [name, rel] of Object.entries(artifacts)) {
  ...
  console.log(`OK ${name} (${artifact.abi.length} entries)`);
}
